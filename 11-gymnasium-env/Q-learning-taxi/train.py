...
def train_agent(q_table, env, num_episodes):
    for i in range(num_episodes):
        ...
        print(f"Time steps: {step}, Penalties: {num_penalties}, Reward: {total_reward}")
...
@click.option('--save-path', default="taxi3.pickle", help='Path to save the Q-table dump', show_default=True)
def main(num_episodes, save_path):
    env = gym.make("Taxi-v3")
    save_path=path.join(path.dirname(__file__), "data/"+save_path)
